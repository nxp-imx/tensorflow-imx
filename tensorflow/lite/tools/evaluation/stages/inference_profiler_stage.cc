...
TfLiteStatus InferenceProfilerStage::Init(
  ...
  reference_stage_.reset(new TfliteInferenceStage(reference_config_));
  ...
  LOG(INFO) << "Test interpreter has been initialized. Using model " << ref_model_path << ", delegate " << ref_delegate_name << ", " << ref_num_threads << " threads and " << ref_invocations << " invocations pre run.";
