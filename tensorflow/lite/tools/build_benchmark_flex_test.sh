...
echo "[Flex Delegate Builder] Bazel target: ${BZL_TARGET}"
echo "[Flex Delegate Builder] Output base: ${BZL_OUTPUT_BASE}"

# Generate the flex op files
if [ ${FLAG_REDUCED} = true ]; then
  echo "[Flex Delegate Builder] Generate flex op files using :fetch_flex_files"
    bazel --output_base=${BZL_OUTPUT_BASE} \
        run //tensorflow/lite/delegates/flex/test:fetch_flex_files
fi

# Build the target
bazel --output_base=${BZL_OUTPUT_BASE} \
  build -c opt --cxxopt=--std=c++17 --config=monolithic --config=elinux_aarch64 \
  --host_crosstool_top=@bazel_tools//tools/cpp:toolchain \
  ${BZL_TARGET}

echo "[Flex Delegate Builder] bin out: "
ls ${BZL_OUTPUT_BASE}/${BZL_OUTPUT_INTER_PATH}${BZL_TARGET_LOC}/${BZL_TARGET_BNAME}_${BZL_TARGET_LINK}${BZL_TARGET_SIZE}
if [ ${FLAG_DYN_LINK} = true ]; then
  echo "[Flex Delegate Builder] .so out: "
  ls ${BZL_OUTPUT_BASE}/${BZL_OUTPUT_INTER_PATH}${BZL_TARGET_LOC}/libtensorflowlite_flex_${BZL_TARGET_SIZE}.so
fi
