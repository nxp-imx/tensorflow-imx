...
function generate_flex_buildfiles {
  ...
  message=('# Source: '$(basename "$0"))
  if [ ! -z ${MODEL_NAMES} ]; then
    message+=(
      ''
      'load("@bazel_skylib//rules:write_file.bzl", "write_file")'
      'load("@org_tensorflow//tensorflow/lite/delegates/flex:build_def.bzl", "generate_flex_kernel_header", "load_flex_kernel_header")'
      ''
      '# Variables controlling generation'
  ...
  if [ ${STATIC_LINK_FLAG} = true ]; then
    ...
    if [ ! -z ${MODEL_NAMES} ]; then
      message+=('    '$(generate_list_field "models" $MODEL_NAMES))
      message+=('    kernel_headers = loaded_headers,')
    fi
    message+=(
      ')'
      ''
  ...
  if [ ${BUILD_BENCHMARK} = true ]; then
    message+=('load("@org_tensorflow//tensorflow:tensorflow.bzl", "tf_cc_binary")')
    message+=('load("@org_tensorflow//tensorflow/lite:build_def.bzl", "tflite_copts", "tflite_copts_warnings", "tflite_linkopts")')
    if [ ${STATIC_LINK_FLAG} = true ]; then
      message+=(
        ''
...
function build_libs {
  ...
  if [ ! -z ${MODEL_NAMES} ]; then
    bazel --output_base=${BZL_OUTPUT_BASE} run  \
      //tmp:fetch_flex_files
  fi
